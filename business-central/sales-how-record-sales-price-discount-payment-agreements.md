...
## <a name="using-sales-and-purchase-price-lists"></a>Using sales and purchase price lists
...
### <a name="converting-existing-prices-when-you-turn-on-the-pricing-feature-update"></a>Converting existing prices when you turn on the pricing feature update
...
#### <a name="create-sales-price-lines-based-on-the-unit-price"></a>Create sales price lines based on the unit price
...
#### <a name="create-sales-price-lines-based-on-existing-price-lists"></a>Create sales price lines based on existing price lists
...
1. [!INCLUDE[prod_short](includes/prod_short.md)] checks the combination of the bill-to customer and the item and then calculates the applicable unit price and line discount percentage, using the following criteria:
    ...
    * Is a unit of measure code specified? If so, [!INCLUDE[prod_short](includes/prod_short.md)] checks for prices/discounts with the same unit of measurement code, and prices/discounts with no unit of measurement code.
...
2. In the **Currency Code** field, enter the code for a currency that the invoice discount terms on the line apply to. Leave the field blank to set up invoice discount terms in GBP.
