...
The sequence of the calculations in your plan is important for getting the project done in a reasonable amount of time. The prioritisation of requirements and resources also plays an important role in getting the best results.  
...
You can also speed up low-level code calculations by turning on the **Optimise low-level code calculation** toggle on the **Manufacturing Setup** page.
...
> [!NOTE]  
> In case of operation splitting, the set-up time is only assigned one time because it's assumed that some manual adjustment is done to optimise the schedule.  
